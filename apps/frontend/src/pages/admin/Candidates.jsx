...
export default function Candidates() {
  ...
  const canDelete = ['SUPER_ADMIN','ADMIN','ADMINISTRATOR','HEAD_ADMIN','HR','MANAGER'].includes(normalizedRole);
  ...
  async function deleteCandidate(candidate) {
    const confirmed = window.confirm(
      `Delete ${candidate.full_name}?\n\nThe candidate, sourcing history and placement history will be removed. Existing invoice snapshots will remain.`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setError(requestError.response?.data?.message || 'Candidate could not be deleted.');
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="module-page">
      ...
      <div className="candidate-list">
        {loading ? <div className="card">Loading candidates...</div> : candidates.length === 0 ? <div className="card empty-state">No candidates found.</div> : candidates.map((candidate) => <article className="card candidate-card" key={candidate.id}>
          <div className="candidate-card-head"><div className="candidate-avatar"><UserRound size={22}/></div><div><h3>{candidate.full_name}</h3><p>{candidate.email || candidate.phone || 'No contact'} Â· Enrolled by {candidate.enrolled_by_name || 'SRSB'}</p></div><div className="row-actions"><button className="btn btn-secondary" type="button" onClick={() => selectCandidate(candidate)}>View Journey</button><button className="icon-btn" type="button" title="Edit candidate" onClick={() => openEdit(candidate)}><Pencil size={17}/></button>{canDelete && <button className="icon-btn danger" type="button" title="Delete candidate" disabled={saving} onClick={() => deleteCandidate(candidate)}><Trash2 size={17}/></button>}</div></div>
          <div className="candidate-info-grid"><span><strong>Source:</strong> {label(candidate.candidate_source)}</span><span><strong>Enrolled:</strong> {formatDate(candidate.enrollment_date)}</span><span><strong>Sourcing records:</strong> {candidate.applications.length}</span><span><strong>Placements:</strong> {candidate.history_count || 0}</span><span><strong>Latest company:</strong> {candidate.latest_company || 'Not placed'}</span><span><strong>Latest status:</strong> {candidate.latest_employment_status ? label(candidate.latest_employment_status) : 'â€”'}</span></div>
          {candidate.applications.length > 0 && <div className="candidate-stage-strip">{candidate.applications.map((application) => <div key={application.application_id}><Building2 size={15}/><span>{application.company_name} â€” {application.job_role}</span><select value={application.stage || 'SOURCED'} onChange={(event) => updateStage(application.application_id, event.target.value, application.assigned_recruiter_id)}>{stages.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select></div>)}</div>}
        </article>)}
      </div>

      {selected && <div className="modal-overlay"><div className="modal-card modal-wide">
        <div className="section-heading"><div><h2>{selected.full_name} â€” Recruitment Journey</h2><p className="page-subtitle">Separate sourcing history from the company where the candidate finally joined.</p></div><button className="icon-btn" type="button" onClick={() => setSelected(null)}><X size={20}/></button></div>
