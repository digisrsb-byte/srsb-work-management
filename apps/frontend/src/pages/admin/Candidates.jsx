...
const emptyCandidate = {
  ...
  enrollmentDate: indiaDateValue(), currentLocation: '', preferredLocation: '',
  totalExperience: '', currentCtc: '', expectedCtc: '', noticePeriodDays: '', skills: '', openingId: '',
  assignedRecruiterId: '', sourcedDate: indiaDateValue(), sourcingNotes: '', stage: 'SOURCED'
};
const emptySourcing = { openingId: '', assignedRecruiterId: '', sourcedDate: indiaDateValue(), sourcingNotes: '', stage: 'SOURCED' };
...
export default function Candidates() {
  ...
  const canDelete = ['SUPER_ADMIN','ADMIN','HR'].includes(normalizedRole);
  ...
  function startPlacementForApplication(candidate, application) {
    setSelected(candidate);
    ...
    setPlacementForm({
      ...
      position: application.job_role || '',
      location: application.requirement_location || '',
      recruiterId: application.assigned_recruiter_id || '',
      joiningDate: indiaDateValue(),
      employmentStatus: 'JOINED'
    });
    setShowPlacement(true);
    setMessage('Complete the joining date and Billing CTC to confirm this placement.');
    setError('');
  }

  async function updateStage(candidate, application, nextStage) {
    if (nextStage === 'JOINED' && !application.application_placement_id) {
      startPlacementForApplication(candidate, application);
      return;
    }

    try {
      await api.put(`/candidates/applications/${application.application_id}/stage`, {
        stage: nextStage,
        assignedRecruiterId: application.assigned_recruiter_id || undefined
      });
      ...
      setError(requestError.response?.data?.message || 'Stage could not be updated.');
    }
  }
