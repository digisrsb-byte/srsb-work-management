...
function findUpdaterAssetByName(release, requestedName) {
  const assets = Array.isArray(release?.assets) ? release.assets : [];
  const safeRequestedName = String(requestedName || '').trim();

  if (
    !/^[a-zA-Z0-9._-]+$/.test(safeRequestedName) ||
    !/\.(?:yml|yaml|exe|blockmap)$/i.test(safeRequestedName)
  ) {
    throw new AppError('Invalid updater asset name.', 400);
  }

  return assets.find(
    (asset) =>
      String(asset?.name || '').toLowerCase() ===
      safeRequestedName.toLowerCase()
  ) || null;
}

export const downloadUpdaterProviderAsset = asyncHandler(async (req, res) => {
  const requestedName = String(req.params.assetName || '').trim();
  const release = await fetchLatestGithubRelease({
    force: requestedName.toLowerCase() === 'latest.yml' && req.query.refresh === '1'
  });

  const asset = findUpdaterAssetByName(release, requestedName);

  if (!asset) {
    throw new AppError(
      'The requested updater file is not part of the latest approved release.',
      404
    );
  }

  const response = await fetch(
    repositoryApiUrl(`/releases/assets/${asset.id}`),
    {
      headers: githubHeaders('application/octet-stream'),
      redirect: 'manual',
      signal: AbortSignal.timeout(30000)
  ...
  if ([301, 302, 303, 307, 308].includes(response.status)) {
    ...
    if (!location) {
      throw new AppError(
        'The update server did not provide an asset download location.',
        502
      );
    }
  ...
  if (!response.ok || !response.body) {
    const githubMessage = await readGithubError(response);
    throw new AppError(
      `The updater asset could not be downloaded: ${githubMessage}`,
      502
    );
  }
